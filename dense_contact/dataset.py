...
GRID_Y = np.linspace(0, 16, 33, dtype=np.float32)

GRID_H = len(GRID_Y)   # 33
...
class TactileDataset(Dataset):
    ...
    def __init__(self, csv_path: str, sessions: list[str], train: bool = True, stride: int = 5):
        ...
        # Subset to requested sessions, then apply depth-aware sampling.
        # Frames where δ < 3 mm have contact diameter < 11 mm — smaller than
        # the 16 mm Y range, so Y position is resolvable. These are sampled
        # densely (stride=2). Deeper frames (contact spills beyond Y range)
        # are sampled sparsely (stride=10) so the model still sees the full
        # depth range but shallow contacts dominate.
        subset = full_df[full_df['session'].isin(sessions)]

        def _depth_sample(g):
            shallow = g[g['displacement_mm'] < 3.0].iloc[::2]
            deep    = g[g['displacement_mm'] >= 3.0].iloc[::10]
            return pd.concat([shallow, deep]).sort_index()

        subset = subset.groupby('session', group_keys=False).apply(_depth_sample)
...
def _cloud_path(image_path: str) -> str:
    """Map an image path to its corresponding point-cloud .npy path."""
    return (image_path
...
class PointCloudDataset(Dataset):
    """
    Dataset for PointCloudNet: returns (image, point_cloud) pairs.

    Each point cloud is a (PC_N_PTS, 3) float32 array of the deformed skin
    surface at the structured 32×64 grid, generated by generate_pointclouds.py.

    Args:
        csv_path : path to dataset.csv
        sessions : list of session strings to include
        train    : whether to apply training augmentations
    """
    ...
    def __init__(self, csv_path: str, sessions: list[str], train: bool = True):
        ...
        # Same depth-aware sampling as TactileDataset
        subset = full_df[full_df['session'].isin(sessions)]
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        cloud = torch.from_numpy(
            np.load(_cloud_path(row['image_path'])).astype(np.float32)
        )  # (PC_N_PTS, 3)
        ...
        return {
            ...
            'displacement_raw': torch.tensor(delta, dtype=torch.float32),
            'force_raw':        torch.tensor(force, dtype=torch.float32),
            'loc_x':            torch.tensor(float(row['loc_x']), dtype=torch.float32),
            'loc_y':            torch.tensor(float(row['loc_y']), dtype=torch.float32),
        }
